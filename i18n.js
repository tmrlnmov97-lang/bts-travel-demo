...
(function () {
  ...
  var CSS =
    '.lang,.lang-seg{font-family:inherit}' +
    '.lang{position:relative;display:inline-flex;margin-left:12px;flex:0 0 auto}' +
    '.lang-btn{display:inline-flex;align-items:center;gap:7px;padding:11px 14px;border:1px solid var(--line,#E8EBF2);' +
      'border-radius:12px;background:#fff;color:var(--navy,#0D2035);font-size:13px;font-weight:700;line-height:1;' +
      'cursor:pointer;transition:border-color .2s,color .2s,box-shadow .2s}' +
    '.lang-btn svg{width:16px;height:16px;color:var(--muted,#5E6B7D);flex:0 0 auto}' +
    '.lang-btn:hover{border-color:var(--indigo,#3534F0);color:var(--indigo,#3534F0)}' +
    '.lang-btn:hover svg{color:var(--indigo,#3534F0)}' +
    '.lang-btn .caret{width:9px;height:9px;border-right:2px solid currentColor;border-bottom:2px solid currentColor;' +
      'transform:rotate(45deg) translate(-2px,-2px);opacity:.5;transition:transform .2s}' +
    '.lang.open .lang-btn .caret{transform:rotate(-135deg) translate(-3px,-3px)}' +
    '.lang-menu{position:absolute;top:calc(100% + 8px);right:0;min-width:124px;padding:6px;background:#fff;' +
      'border:1px solid var(--line,#E8EBF2);border-radius:12px;box-shadow:0 22px 44px -20px rgba(13,32,53,.4);' +
      'display:none;flex-direction:column;gap:2px;z-index:120}' +
    '.lang.open .lang-menu{display:flex}' +
    '.lang-menu button{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:9px 11px;' +
      'border:0;border-radius:8px;background:transparent;color:var(--ink,#16263A);font-size:13px;font-weight:600;' +
      'font-family:inherit;text-align:left;cursor:pointer;transition:background .18s,color .18s}' +
    '.lang-menu button:hover{background:var(--mist,#F4F6FB)}' +
    '.lang-menu button.on{color:var(--indigo,#3534F0);font-weight:800}' +
    '.lang-menu button.on::after{content:"";width:6px;height:10px;border-right:2px solid currentColor;' +
      'border-bottom:2px solid currentColor;transform:rotate(45deg);margin-right:2px}' +
    '.lang-seg{display:inline-flex;align-items:center;gap:9px}' +
    '.lang-seg>svg{width:17px;height:17px;color:var(--muted2,#97A0AE);flex:0 0 auto}' +
    '.lang-seg .seg{display:inline-flex;padding:3px;gap:2px;background:var(--mist,#F4F6FB);' +
      'border:1px solid var(--line,#E8EBF2);border-radius:10px}' +
    '.lang-seg .seg button{padding:6px 13px;border:0;border-radius:7px;background:transparent;' +
      'color:var(--muted2,#97A0AE);font-family:inherit;font-size:12px;font-weight:700;letter-spacing:.04em;' +
      'cursor:pointer;transition:background .18s,color .18s}' +
    '.lang-seg .seg button:hover{color:var(--ink,#16263A)}' +
    '.lang-seg .seg button.on{background:var(--navy,#0D2035);color:#fff}' +
    '@media (max-width:600px){.lang{margin-left:8px}.lang-btn{padding:10px 11px;gap:5px}' +
      '.lang-btn .code{display:none}.lang-btn .caret{display:none}}';
  ...
  function buildHeader() {
    ...
    var box = el('<div class="lang" data-no-i18n>' +
      '<button class="lang-btn" type="button" aria-haspopup="true" aria-expanded="false" aria-label="Dil / Язык / Language">' +
        GLOBE + '<span class="code">AZ</span><i class="caret"></i></button>' +
    ...
    if (call) call.insertAdjacentElement('afterend', box); else hdr.appendChild(box);
  ...
  function buildFooter() {
    ...
    var box = el('<div class="lang-seg" data-no-i18n>' + GLOBE + '<div class="seg" role="group" aria-label="Dil / Язык / Language">' +
      LANGS.map(function (l) {
  ...
  function sync(lang) {
    ...
    // textContent yazmaq uşaq düyünləri əvəz edir — yalnız dəyişəndə yaz
    if (code && code.textContent !== lang.toUpperCase()) code.textContent = lang.toUpperCase();
  ...
  function init() {
    ...
    // sonradan əlavə olunan blokları da tərcümə et
    // (cms.js kartları, mobil menyu, tur səhifəsinin bölmələri)
    if (window.MutationObserver) {
      mo = new MutationObserver(function (muts) {
        ...
        muts.forEach(function (m) {
          ...
          // öz idarəetmə elementlərimizə toxunmuruq — əks halda sonsuz dövrə yaranır
          if (host && host.closest && host.closest('[data-no-i18n]')) return;

          if (m.type === 'characterData') {
            // Biz yazanda müşahidəçi söndürülüdür — deməli bu yazını kənar skript edib.
            // Burada müqayisə etmək olmaz: cms.js çox vaxt eyni azərbaycanca mətni geri
            // yazır, onda __az ilə nodeValue üst-üstə düşür və yoxlama tərcüməni ötürərdi.
  ...
  window.btsI18n = {
    ...
    apply: apply,
    translate: translate   // ayrıca sətri yoxlamaq üçün
  };
